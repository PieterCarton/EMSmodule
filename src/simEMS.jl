...
macro safe_assign(target, value, msg)
    quote
        local val = $(esc(value))
        if isnan(val)
            @warn $(esc(msg))
...
function perfModel_matching(stgAsset::BESSData)
    # check if the performance model is already a PBROMParams
    if stgAsset.PerfParameters isa PBROMParams
...
function agingModel_matching(stgAsset::BESSData, perfModel::PBROMParams)
    if stgAsset.AgingParameters isa JinAgingParams
...
function update_stgAsset_deg!(stgAsset::BESSData, results::Dict, key::String; typeOpt::String="MPC")
    ...
    if typeOpt == "MPC"
        # shift = 1;
        # # validate if NaN before updating the stgAsset. Trigger a warning, don't update the stgAsset and continue
        # isnan(Qsa[shift+1]/Qsan) ? warn("NaN detected in SoHQ update") : (stgAsset.GenInfo.SoHQ = copy(Qsa[shift+1]/Qsan));
        # # stgAsset.GenInfo.SoHQ = copy(Qsa/Qsan);
        # isnan(R0) ? warn("NaN detected in SoHR0 update") : (stgAsset.GenInfo.SoHR0 = copy(R0));
        # if typeof(stgAsset.PerfParameters) == CIDRAPBROMPerfParams
        #     isnan(Qloss[shift+1] / Qsa0) ? warn("NaN detected in θ_100 update") : 
        ...
        #     isnan(R0) ? warn("NaN detected in RFilm update") : (stgAsset.PerfParameters.Cell.Neg.RFilm = copy(R0));
        # elseif typeof(stgAsset.PerfParameters) == ECMPerfParams
        #     isnan(R0) ? warn("NaN detected in R0Param update") : (stgAsset.PerfParameters.R0Param = copy([R0]));
        # end
        # if typeof(stgAsset.AgingParameters) == JinAgingParams
        #     isnan(Qloss[shift+1] / Qsa0) ? warn("NaN detected in z100p update") :
        ...
        #     isnan(δSEI) ? warn("NaN detected in δSEI0 update") : (stgAsset.AgingParameters.δSEI0 = copy(δSEI));
        #     isnan(εₑ) ? warn("NaN detected in εₑ0 update") : (stgAsset.AgingParameters.εₑ0 = copy(εₑ));
        # end
        # Then replace the repetitive code:
        shift = 1;
        # Validate and update values using the macro
        @safe_assign stgAsset.GenInfo.SoHQ Qsa[shift+1]/Qsan "NaN detected in SoHQ update"
        @safe_assign stgAsset.GenInfo.SoHR0 R0 "NaN detected in SoHR0 update"

        if typeof(stgAsset.PerfParameters) == PBROMParams
            @safe_assign stgAsset.PerfParameters.Cell.Neg.θ_100 (stgAsset.PerfParameters.Cell.Neg.θ_100 - Qloss[shift+1] / Qsa0) "NaN detected in θ_100 update"
            @safe_assign stgAsset.PerfParameters.Cell.Neg.RFilm R0 "NaN detected in RFilm update"
        elseif typeof(stgAsset.PerfParameters) == ECMPerfParams
            @safe_assign stgAsset.PerfParameters.R0Param [R0] "NaN detected in R0Param update"
        end

        if typeof(stgAsset.AgingParameters) == JinAgingParams
            @safe_assign stgAsset.AgingParameters.z100p (stgAsset.AgingParameters.z100p - Qloss[shift+1] / Qsa0) "NaN detected in z100p update"
            @safe_assign stgAsset.AgingParameters.δSEI0 δSEI "NaN detected in δSEI0 update"
            @safe_assign stgAsset.AgingParameters.εₑ0 εₑ "NaN detected in εₑ0 update"
        end
...
function update_stgAsset_deg!(stgAsset::BESSData, results::Dict, key::String, typeOpt::CT_MPC)
    # This function updates the storage asset with the degradation results
    # extract from the results dictionary the degradation results
    shift = 1;
    Qsa = length(results["Q$key"]) > 1 ? copy(results["Q$key"][shift+1]) : copy(results["Q$key"])
    R0 = copy(results["R0$key"])
    Qsan = copy(stgAsset.GenInfo.initQ); # rated capacity
    SoHQ = copy(stgAsset.GenInfo.SoHQ); # initial SoHQ
    Qsa0 = Qsan * SoHQ; # initial capacity
    ...
    εₑ = copy(results["εₑ$key"])    
    # Validate and update values using the macro
    @safe_assign stgAsset.GenInfo.SoHQ Qsa/Qsan "NaN detected in SoHQ update"
    @safe_assign stgAsset.GenInfo.SoHR0 R0 "NaN detected in SoHR0 update"
    if typeof(stgAsset.PerfParameters) == PBROMParams
        @safe_assign stgAsset.PerfParameters.Cell.Neg.θ_100 (stgAsset.PerfParameters.Cell.Neg.θ_100 - Qloss / Qsa0) "NaN detected in θ_100 update"
        @safe_assign stgAsset.PerfParameters.Cell.Neg.RFilm R0 "NaN detected in RFilm update"
    elseif typeof(stgAsset.PerfParameters) == ECMPerfParams
        @safe_assign stgAsset.PerfParameters.R0Param [R0] "NaN detected in R0Param update"
    end
    if typeof(stgAsset.AgingParameters) == JinAgingParams
        @safe_assign stgAsset.AgingParameters.z100p (stgAsset.AgingParameters.z100p - Qloss / Qsa0) "NaN detected in z100p update"
        @safe_assign stgAsset.AgingParameters.δSEI0 δSEI "NaN detected in δSEI0 update"
        @safe_assign stgAsset.AgingParameters.εₑ0 εₑ "NaN detected in εₑ0 update"
    end
    return stgAsset;
...
function update_stgAsset_deg!(stgAsset::BESSData, results::Dict, key::String, typeOpt::DayAhead)
    # This function updates the storage asset with the degradation results
...
function simulate_storage_asset_deg!(stgAsset::BESSData,
    ...
    # final state for the stgAsset
    update_stgAsset_deg!(stgAsset, results, key, typeOpt)
    return stgAsset, results
end
...
function simulate_storage_asset!(stgAsset::BESSData, results::Dict, key::String; typeOpt::String="MPC")
    ...
    check = false; skip = false;
    if any(isnan.(stgVars.Iapp))
        @warn("NaNs in the solution, drop simulation results and keeping optimization setpoints,.")
        skip = true
    end
    ...
    if any(stgVars.Cell_SOC .< SList[2]) || any(stgVars.Cell_SOC .> SList[1])
        println("SoC out of bounds.")
        iMin = stgVars.Cell_SOC .< SList[2]
        iMax = stgVars.Cell_SOC .> SList[1]
        # replace the SoC values that are out of bounds
        stgVars.Cell_SOC[iMin] .= (SList[2] .+ 1e-4);
        stgVars.Cell_SOC[iMax] .= (SList[1] .- 1e-4);
    ...
    if !skip
        # if there was an issue with the simulation, we skip updating the results
        if typeOpt == "MPC"
        ...
        # For ECMs
        # if PerfParams ≠ PBROMParams then update straight from the optimization results
...
function get_PBROM_matrices(perfModel::CIDRAPBROMPerfParams, cellID::String)
    # This function returns the PBROM matrices for a given cellID
    # The inputs are:
    # - perfModel: a CIDRAPBROMPerfParams object containing the performance model parameters
    # - cellID: the cell ID of the storage asset
    # This is the slow method without precomputed matrices, calling LiiBRA.Realise() each time
    cellID == "SYNSANYO" ? SList = [1.0, 0.15] : SList = [0.95, 0.1] # List of SOC points for model generation
    Sₑ = 4 # Spatial points in electrolyte
    Sₛ = 2 # Spatial point in solid
    Base.invokelatest(Spatial!, perfModel.Cell, Sₑ, Sₛ)
    A, B, C, D = Base.invokelatest(Realise, perfModel.Cell, SList)
    return A, B, C, D, SList
end
function get_PBROM_matrices(perfModel::ROMPerfParams, cellID::String)
    # This function returns the PBROM matrices for a given cellID
    # The inputs are:
    # - perfModel: a ROMPerfParams object containing the performance model parameters
    # - cellID: the cell ID of the storage asset
    # This is the fast method with precomputed matrices
    A = perfModel.A;
    B = perfModel.B;
    C = perfModel.C;
    D = perfModel.D;
    SList = perfModel.SList;
    return A, B, C, D, SList
end

function simulate_storage_asset!(stgAsset::BESSData, results::Dict, key::String, typeOpt::CT_MPC)
    # This function simulates a storage asset for a given PsaOpt using LiiBRA
    # The inputs are:
    # - stgAsset: a BESSData object containing the storage asset parameters
    # - results: a dictionary containing the setpoints of the controller
    ...
    # Update the PBROM matrices
    SoC0 = copy(stgAsset.GenInfo.SoC0); # Starting SOC
    A, B, C, D, SList = get_PBROM_matrices(perfModel, stgAsset.cellID)
    ...
    # modify the key for the EV case. Check if it is "evTot" or "evTot[$n]"
    # if it is "evTot[$n]" then we need to change it to "ev[$n]"
    occursin("evTot", key) ? key = replace(key, "evTot" => "ev") : nothing
    if !skip
        # if there was an issue with the simulation, we skip updating the results
        results["SoC$key"][shift+2] = copy(stgVars.Cell_SOC[end]);
        # stgAsset.GenInfo.SoC0 = copy(results["SoC$key"][2])
        ...
        # For ECMs
        # if PerfParams ≠ PBROMPerfParams then update straight from the optimization results
        # not the most accurate but it is the best estimate
        typeof(stgAsset.PerfParameters) == ECMPerfParams ? stgAsset.PerfParameters.iRn0[1] = copy(stgVars.ϕ_ẽ1[2,end]+stgVars.ϕ_ẽ2[2,end]) : nothing; # should I recalculate results["iR1$key"]??
    end
    ...
    # now use the current i to calculate the ageing of the storage asset
    # Update θsa, the ageing of the storage asset
    simulate_storage_asset_deg!(stgAsset, perfModel, results, key, typeOpt)

    return stgAsset, results
end

function simulate_storage_asset!(stgAsset::BESSData, results::Dict, key::String, typeOpt::DayAhead)
    # This function simulates a storage asset for a given PsaOpt using LiiBRA
    # The inputs are:
    # - stgAsset: a BESSData object containing the storage asset parameters
    # - results: a dictionary containing the setpoints of the controller
...
function simulate_storage_asset!(stgAsset::TESSData, results::Dict, key::String, typeOpt::CT_MPC)
    shift = 1;
    # take the length from results because its the one handled by handleInfeasible()
    Δt = copy(results[:"t"][2]-results[:"t"][1]); # setpoint timestep
...
function simulate_storage_asset!(stgAsset::TESSData, results::Dict, key::String, typeOpt::DayAhead)
    # take the length from results because its the one handled by handleInfeasible()
    Δt = copy(results[:"t"][2]-results[:"t"][1]); # setpoint timestep
    shift = Int(24*3600/Δt)-1;
    # Inputs:
...
function simulate_storage_asset!(stgAsset::TESSData_T, results::Dict, key::String; typeOpt::String="MPC")
    @assert typeOpt ∈ ["MPC", "day-ahead"];
    typeOpt == "MPC" ? shift = 1 : nothing;
    # take the length from results because its the one handled by handleInfeasible() it will fail
    Δt = copy(results[:"t"][2]-results[:"t"][1]); # setpoint timestep
    typeOpt == "day-ahead" ? shift = Int(24*3600/Δt)-1 : nothing;
    ...
    Δt = tk[2]-tk[1]; # setpoint timestep
    c = stgAsset.c/3600/1e3; # specific heat capacity [kWh/kg.K]
    m = stgAsset.m; # mass [kg]
    Q̇sd = 0.001 * stgAsset.Q # self-discharge rate [kW]

    if typeOpt == "MPC"
        # easy simulation of a first order bucket model
        # PsaOpt = results["P$key"][shift];
        Q̇ₕₚᵗᵉˢˢ = results["Q̇ₕₚᵗᵉˢˢ"][shift+1];
        Q̇ₜₑₛₛᴰ = results["Q̇ₜₑₛₛᴰ"][shift+1];
        
        T0 = copy(stgAsset.T0); # Starting T
        # easy simulation of a first order bucket model
        Q̇ₜₑₛₛ⁻ = Q̇ₕₚᵗᵉˢˢ - Q̇ₜₑₛₛᴰ - Q̇sd # a priori Q̇ₜₑₛₛ
        Q̇ₜₑₛₛ = copy(Q̇ₜₑₛₛ⁻)
        # Enforce lower T bound
        if T0 + Q̇ₜₑₛₛ⁻ * Δt / c / m / 3600 < stgAsset.TLim[1]
            Q̇ₜₑₛₛ⁺ = (stgAsset.TLim[1] - T0) * c * m * 3600 / Δt
            ΔQ = Q̇ₜₑₛₛ⁺ - Q̇ₜₑₛₛ⁻ #adjustment
            results["Q̇ₜₑₛₛᴰ"][shift+1] = copy(Q̇ₜₑₛₛᴰ + ΔQ) # adjust the Q̇ₜₑₛₛᴰ
            Q̇ₜₑₛₛ = copy(Q̇ₜₑₛₛ⁺)
        end
        stgAsset.T0 = copy(T0 .+ Q̇ₜₑₛₛ .* Δt ./ c ./ m ./ 3600);
    else # typeOpt == "day-ahead"
        # easy simulation of a first order bucket model
        # PsaOpt = results["P$key"][1:shift+1];
        Q̇ₕₚᵗᵉˢˢ = results["Q̇ₕₚᵗᵉˢˢ"][1:shift+1];
        Q̇ₜₑₛₛᴰ = results["Q̇ₜₑₛₛᴰ"][1:shift+1];
        Ttess = zeros(shift+1); # initialize the T
        T0 = copy(stgAsset.T0); # Starting SOC
        Ttess[1] = copy(T0); # initial SoC
        Q̇ₜₑₛₛ = Q̇ₕₚᵗᵉˢˢ .- Q̇ₜₑₛₛᴰ .- Q̇sd
        # easy simulation of a first order bucket model
        results["T$key"] = copy(T0 .+ cumsum(Q̇ₜₑₛₛ) .* Δt ./ c ./ m ./ 3600);
        # Save in the results Dict
        # results["SoC$key"] = copy(SoCsa);
        # results["P$key"] = copy(PsaOpt);
        # save the last SoC for the next initial SoC
        stgAsset.T0 = copy(results["T$key"])[end];
    end
    return stgAsset, results
end

function simulate_storage_asset!(stgAsset::TESSData_T, results::Dict, key::String, typeOpt::CT_MPC)
    shift = 1
    # take the length from results because its the one handled by handleInfeasible() it will fail
    Δt = copy(results[:"t"][2]-results[:"t"][1]); # setpoint timestep
    ...
    # Enforce lower T bound
    if T0 + Q̇ₜₑₛₛ⁻ * Δt / c / m / 3600 < stgAsset.TLim[1]
        Q̇ₜₑₛₛ⁺ = (stgAsset.TLim[1] - T0) * c * m * 3600 / Δt # < 0
        ΔQ = Q̇ₜₑₛₛ⁺ - Q̇ₜₑₛₛ⁻ #adjustment not supplied by the TESS
        # results["Q̇ₜₑₛₛᴰ"][shift+1] = copy(Q̇ₜₑₛₛᴰ + ΔQ) # adjust the Q̇ₜₑₛₛᴰ
        # reduce the heatflow from TESS to Demand
        results["Q̇ₜₑₛₛᴰ"][shift+1] = copy(-Q̇ₜₑₛₛ⁺ + Q̇sd) # adjust the Q̇ₜₑₛₛᴰ
        # don't charge the TESS
        results["Q̇ₕₚᵗᵉˢˢ"][shift+1] = 0. # adjust the Q̇ₕₚᵗᵉˢˢ and set it to 0
        # the heatflow not supplied by the TESS has to be supplied by the heatpump
        results["Q̇ₕₚᴰ"][shift+1] += copy(ΔQ)
        Q̇ₜₑₛₛ = copy(Q̇ₜₑₛₛ⁺)
    end
    stgAsset.T0 = clamp(copy(T0 .+ Q̇ₜₑₛₛ .* Δt ./ c ./ m ./ 3600), stgAsset.TLim[1], stgAsset.TLim[2]);
    results["T$key"][shift+1] = copy(stgAsset.T0);
    return stgAsset, results
end

function simulate_storage_asset!(stgAsset::TESSData_T, results::Dict, key::String, typeOpt::DayAhead)
    # take the length from results because its the one handled by handleInfeasible() it will fail
    Δt = copy(results[:"t"][2]-results[:"t"][1]); # setpoint timestep
    shift = Int(24*3600/Δt)-1
    # Inputs:
...
function simulate_storage_asset!(stgAsset::BuildingData, results::Dict, key::String; typeOpt::String="MPC")
    @assert typeOpt ∈ ["MPC", "day-ahead"];
    typeOpt == "MPC" ? shift = 1 : nothing;
    # take the length from results because its the one handled by handleInfeasible() it will fail
    Δt = copy(results[:"t"][2]-results[:"t"][1]); # setpoint timestep
    ...
    tk = copy(results[:"t"][1:shift+1]); 
    Δt = tk[2]-tk[1]; # setpoint timestep
    @unpack_BuildingData stgAsset
    Cair *= 3600 # to [kWs/kg.K]
    Cb *= 3600 # to [kWs/K]
    cf /= 1000 # to [kWs/kg.K]
    rb /= 3600 # to [1/s]
    ṁf = mdot; # mass flow rate [kg/s]
    c = Cb + Vb * ρair * Cair # in [kWs/K]
        
    if typeOpt == "MPC"
        # easy simulation of a first order bucket model
        Q̇ir = results["Q̇ir"][shift+1];
        Q̇ₕₚᴰ = results["Q̇ₕₚᴰ"][shift+1];
        Q̇ₜₑₛₛᴰ = haskey(results,"Q̇ₜₑₛₛᴰ") ? results["Q̇ₜₑₛₛᴰ"][shift+1] : 0. ;
        Q̇loss = results["Q̇loss"][shift+1]; # check
        T0 = copy(stgAsset.Tin0); # Starting T
        
        # easy simulation of a first order bucket model
        Q̇ᵢₙ = Q̇ir .+ Q̇ₜₑₛₛᴰ + Q̇ₕₚᴰ - Q̇loss
        stgAsset.Tin0 = copy(T0 .+ Q̇ᵢₙ .* Δt ./ c);
        ...
        # From Joel´s paper
        results["T2"][shift+2] = results["T4"][shift+1] + Q̇ₕₚᴰ * Δt / (ṁf * cf)
        results["T3"][shift+2] = results["T4"][shift+1] + Q̇ₜₑₛₛᴰ * Δt / (ṁf * cf)
        results["T4"][shift+2] = results["T4"][shift+1] - (Q̇ir .+ Q̇ₜₑₛₛᴰ + Q̇ₕₚᴰ - Q̇loss) * Δt / (ṁf * cf)
    else # typeOpt == "day-ahead"
        # easy simulation of a first order bucket model
        Q̇ir = results["Q̇ir"][1:shift+1];
        Q̇ₕₚᴰ = results["Q̇ₕₚᴰ"][1:shift+1];
        Q̇ₜₑₛₛᴰ = haskey(results, "Q̇ₜₑₛₛᴰ") ? results["Q̇ₜₑₛₛᴰ"][1:shift+1] : zeros(shift+1);
        Q̇loss = results["Q̇loss"][1:shift+1]; # check
        
        Tin = zeros(shift+1); # initialize the T
        T0 = copy(stgAsset.Tin0); # Starting SOC
        Tin[1] = copy(T0); # initial SoC
        Q̇ᵢₙ = Q̇ir .+ Q̇ₜₑₛₛᴰ .+ Q̇ₕₚᴰ .- Q̇loss
        # easy simulation of a first order bucket model
        results["Tin"] = copy(T0 .+ cumsum(Q̇ᵢₙ) .* Δt ./ c);
        # Save in the results Dict
        # results["SoC$key"] = copy(SoCsa);
        # results["P$key"] = copy(PsaOpt);
        # save the last SoC for the next initial SoC
        stgAsset.Tin0 = copy(results["Tin"])[end];
        results["T2"][2:shift+1] = results["T4"][1:shift] .+ Q̇ₕₚᴰ[1:shift] * Δt / (ṁf * cf)
        results["T3"][2:shift+1] = results["T4"][1:shift] .+ Q̇ₜₑₛₛᴰ[1:shift] * Δt / (ṁf * cf)
        results["T4"][2:shift+1] = results["T4"][1:shift] .- (Q̇ir .+ Q̇ₜₑₛₛᴰ + Q̇ₕₚᴰ - Q̇loss)[1:shift] * Δt / (ṁf * cf)
    end
    return stgAsset, results
end

function simulate_storage_asset!(stgAsset::BuildingData, results::Dict, key::String, typeOpt::CT_MPC)
    shift = 1
    # take the length from results because its the one handled by handleInfeasible() it will fail
    Δt = copy(results[:"t"][2]-results[:"t"][1]); # setpoint timestep
    ...
    # easy simulation of a first order bucket model
    Q̇ᵢₙ = Q̇ir .+ Q̇ₜₑₛₛᴰ + Q̇ₕₚᴰ - Q̇loss
    stgAsset.Tin0 = clamp(copy(T0 .+ Q̇ᵢₙ .* Δt ./ c), 273., 273. + 40.);
    # Calculate piping Ts
    # ∂.(Tⁱⁿ, t) * mf * cf .== Q̇ - ηₕₚ * ṁf * cf * (Tⁱⁿ - Tᵒᵘᵗ)
    # ∂.(T2, t) * mf * cf .==  Q̇ₕₚᴰ - η * ṁf * cf * (T2 - T3)
    # ∂.(T3, t) * mf * cf .==  Q̇ₜₑₛₛᴰ - η * ṁf * cf * (T3 - T4)
    # ∂.(T4, t) * mf * cf .==  -(Q̇ir .+ Q̇ₜₑₛₛᴰ + Q̇ₕₚᴰ - Q̇loss) - η * ṁf * cf * (T4 - T2)
    results["Tin"][shift+1] = copy(stgAsset.Tin0);
    # From Joel´s paper
    results["T2"][shift+2] = results["T4"][shift+1] + Q̇ₕₚᴰ * Δt / (ṁf * cf)
    results["T3"][shift+2] = results["T4"][shift+1] + Q̇ₜₑₛₛᴰ * Δt / (ṁf * cf)
    results["T4"][shift+2] = results["T4"][shift+1] - (Q̇ir .+ Q̇ₜₑₛₛᴰ + Q̇ₕₚᴰ - Q̇loss) * Δt / (ṁf * cf)
...
function simulate_storage_asset!(stgAsset::BuildingData, results::Dict, key::String, typeOpt::DayAhead)
    # take the length from results because its the one handled by handleInfeasible() it will fail
    Δt = copy(results[:"t"][2]-results[:"t"][1]); # setpoint timestep
    shift = Int(24*3600/Δt)-1
...
function simTransitionFun_new_th!(results::Dict, data::Dict, s::modelSettings; typeOpt::String="MPC")
    ...
    if typeOpt == "MPC"
        ...
        # from the new thermal actions
        # heat
        Q̇ₜₑₛₛᴰ = haskey(results, "Q̇ₜₑₛₛᴰ") ? results["Q̇ₜₑₛₛᴰ"][shift+1] : 0;
        ThpDⁱⁿ = haskey(results, "ThpDⁱⁿ") ? results["ThpDⁱⁿ"][shift+1] : 0;
        ThpTESSⁱⁿ = haskey(results, "ThpTESSⁱⁿ") ? results["ThpTESSⁱⁿ"][shift+1] : 0;
        # Second, we get the exogenous information.
        # P = P̂ + ϵ
        Ple = data["grid"].loadE[it0]
        # Plt = data["grid"].loadTh[it0]
        Tamb = data["Building"].ambT[it0] .+ 273.15
        ...
        # re-calculate the HP heat and COP
        if results["Q̇ₕₚᴰ"][shift+1] > 1e-4 # HP → D activated
            results["COPhatᴰ"][shift+1] = 7.90471 .* exp(-0.024 .* (ThpDⁱⁿ .- Tamb)) # COP nonlinear relationship
            results["COPhatᵗᵉˢˢ"][shift+1] = 0
        elseif results["Q̇ₕₚᵗᵉˢˢ"][shift+1] > 1e-4 # HP → TESS activated
            results["COPhatᴰ"][shift+1] = 0
            results["COPhatᵗᵉˢˢ"][shift+1] = 7.90471 .* exp(-0.024 .* (ThpTESSⁱⁿ .- Tamb)) # COP nonlinear relationship
        end
        results["Q̇ₕₚᴰ"][shift+1] = results["COPhatᴰ"][shift+1] .* Phpe
        results["Q̇ₕₚᵗᵉˢˢ"][shift+1] = results["COPhatᵗᵉˢˢ"][shift+1] .* Phpe
        # # Add the results to the results Dict
        # haskey(results, "Q̇ₕₚᴰ") ? results["Q̇ₕₚᴰ"][shift+1] = copy(Q̇ₕₚᴰ) : merge!(results, Dict("Q̇ₕₚᴰ" => Q̇ₕₚᴰ));
        # haskey(results, "Q̇ₕₚᵗᵉˢˢ") ? results["Q̇ₕₚᵗᵉˢˢ"][shift+1] = copy(Q̇ₕₚᵗᵉˢˢ) : merge!(results, Dict("Q̇ₕₚᵗᵉˢˢ" => Q̇ₕₚᵗᵉˢˢ));
        ...
        if nEV != 1
            γf = haskey(results, "γf") ? [results["γf"][n][shift+1] for n ∈ 1:nEV] : [0. for n ∈ 1:nEV];
            results["Pg"][shift+1] = copy(Ple + Phpe - PpvMPPT - # data
                Pbess - sum([Pev[n] .* γf[n] for n ∈ 1:nEV]));
        else
            γf = haskey(results, "γf") ? results["γf"][shift+1] : 0.;
            results["Pg"][shift+1] = copy(Ple + Phpe - PpvMPPT - # data
                Pbess - sum([Pev[n] .* γf for n ∈ 1:nEV]));
        end
    else # typeOpt == "day-ahead"
        # First, we get the optimal decisions from our policy function.
        Pbess = haskey(results, "Pbess") ? results["Pbess"][1:shift+1] : zeros(shift+1);
        Pev = haskey(results, "Pev[1]") ? [results["Pev[$n]"][1:shift+1] for n ∈ 1:nEV] : [zeros(shift+1) for n ∈ 1:nEV];
        # check if we have a heat pump
        Phpe = haskey(results, "Phpe") ?  results["Phpe"][1:shift+1] : zeros(shift+1);
        Q̇ₜₑₛₛᴰ = haskey(results, "Q̇ₜₑₛₛᴰ") ? results["Q̇ₜₑₛₛᴰ"][1:shift+1] : zeros(shift+1);
        ThpDⁱⁿ = haskey(results, "ThpDⁱⁿ") ? results["ThpDⁱⁿ"][1:shift+1] : zeros(shift+1);
        ThpTESSⁱⁿ = haskey(results, "ThpTESSⁱⁿ") ? results["ThpTESSⁱⁿ"][1:shift+1] : zeros(shift+1);;
        ...
        Tamb = copy(data["Building"].ambT[it0:itend]) .+ 273.15
        ...
        # re-calculate the HP heat and COP, ensuring complementarity
        for ts ∈ 1:shift+1
            if results["Q̇ₕₚᴰ"][ts] > 1e-4 # HP → D activated
                results["COPhatᴰ"][ts] = 7.90471 .* exp(-0.024 .* (ThpDⁱⁿ[ts] .- Tamb[ts])) # COP nonlinear relationship
                results["COPhatᵗᵉˢˢ"][ts] = 0
            elseif results["Q̇ₕₚᵗᵉˢˢ"][ts] > 1e-4 # HP → TESS activated
                results["COPhatᴰ"][ts] = 0
                results["COPhatᵗᵉˢˢ"][ts] = 7.90471 .* exp(-0.024 .* (ThpTESSⁱⁿ[ts] .- Tamb[ts])) # COP nonlinear relationship
            end
        end
        results["Q̇ₕₚᴰ"][1:shift+1] = results["COPhatᴰ"][1:shift+1] .* Phpe
        results["Q̇ₕₚᵗᵉˢˢ"][1:shift+1] = results["COPhatᵗᵉˢˢ"][1:shift+1] .* Phpe
        # Add the results to the results Dict
        # haskey(results, "Q̇ₕₚᴰ") ? results["Q̇ₕₚᴰ"][1:shift+1] = copy(Q̇ₕₚᴰ) : merge!(results, Dict("Q̇ₕₚᴰ" => Q̇ₕₚᴰ));
        # haskey(results, "Q̇ₕₚᵗᵉˢˢ") ? results["Q̇ₕₚᵗᵉˢˢ"][1:shift+1] = copy(Q̇ₕₚᵗᵉˢˢ) : merge!(results, Dict("Q̇ₕₚᵗᵉˢˢ" => Q̇ₕₚᵗᵉˢˢ));
        # and from the electrical one the grid
        if nEV != 1
            γf = haskey(results, "γf") ? [results["γf"][n][1:shift+1] for n ∈ 1:nEV] : [zeros(shift+1) for n ∈ 1:nEV];
            results["Pg"][1:shift+1] = copy(Ple + Phpe - PpvMPPT - # data
                Pbess - sum([Pev[n] .* γf[n] for n ∈ 1:nEV]));
        else
            γf = haskey(results, "γf") ? results["γf"][1:shift+1] : zeros(shift+1);
            results["Pg"][1:shift+1] = copy(Ple + Phpe - PpvMPPT - # data
                Pbess - sum([Pev[n] .* γf for n ∈ 1:nEV]));
        end
    end
    ...
    # get all the SoC
    storages = []
    haskey(results, "Pbess") ? push!(storages, "BESS") : nothing;
    haskey(results, "Pev[1]") ? push!(storages, "EV") : nothing;
    haskey(results, "Q̇ₜₑₛₛᴰ") ? push!(storages, "TESS") : nothing;
    haskey(results, "Tin") ? push!(storages, "Building") : nothing;
    for sa ∈ storages
        if typeof(data[sa]) <: Vector
            for n ∈ 1:length(data[sa])
                ...
                elseif typeof(data[sa][n]) == TESSData_T
                    key="tess[$n]"
                    stgAsset = data[sa][n];
                elseif typeof(data[sa][n]) == BuildingData
                    key="building"
                    stgAsset = data[sa][n];
                end
            ...
            elseif typeof(data[sa]) == TESSData_T
                key="tess"
                stgAsset = data[sa];
            elseif typeof(data[sa]) == BuildingData
                key="building"
                stgAsset = data[sa];
            end
            # Sₛₐ,ₜ₊₁ = Sₛₐ,ₜᴹ(Sₛₐ,ₜ , Pₛₐ,ₜ* , Wₛₐ,ₜ₊₁)
    ...
    if typeOpt == "MPC"
        ...
        # Pbess = results["Pbess"][shift+1];
        # Phpe = results["Phpe"][shift+1]
        Pbess = haskey(results, "Pbess") ? results["Pbess"][1:shift+1] : 0;
        Phpe = haskey(results, "Phpe") ? results["Phpe"][1:shift+1] : 0;
        Pev = haskey(results, "Pev[1]") ? [results["Pev[$n]"][1:shift+1] for n ∈ 1:nEV] : zeros(nEV);
...
function simTransitionFun_new_th!(results::Dict, data::Dict, s::modelSettings, typeOpt::CT_MPC)
        # This function simulates the transition function Sₐ,ₜ₊₁ = Sₐ,ₜᴹ(Sₐ,ₜ , Pₐ,ₜ* , Wₐ,ₜ₊₁).
    # Sₐ,ₜ: state in timestep t
    # Pₐ,ₜ*: optimal setpoint in timestep t
    # Wₐ,ₜ₊₁: exogenous information in timestep t+1
    ...
    storages = []
    haskey(results, "Tin") ? push!(storages, "Building") : nothing;
    haskey(results, "Pbess") ? push!(storages, "BESSpbrom") : nothing;
    haskey(results, "Pev[1]") ? push!(storages, "EVpbrom") : nothing;
    haskey(results, "Q̇ₜₑₛₛᴰ") ? push!(storages, "TESS") : nothing;
    for sa ∈ storages
        if typeof(data[sa]) <: Vector
            for n ∈ 1:length(data[sa])
                ...
                # Sₛₐ,ₜ₊₁ = Sₛₐ,ₜᴹ(Sₛₐ,ₜ , Pₛₐ,ₜ* , Wₛₐ,ₜ₊₁)
                simulate_storage_asset!(stgAsset, results, key, typeOpt)
            end
        else
            # pick the right key
            if (typeof(data[sa]) == BESSData)
    ...
    # re-balance the power
    if results["Q̇ₕₚᵗᵉˢˢ"][shift+1] > 1e-4
        results["Phpe"][shift+1] = copy(results["Q̇ₕₚᵗᵉˢˢ"][shift+1] ./ results["COPhatᵗᵉˢˢ"][shift+1]);
    elseif results["Q̇ₕₚᴰ"][shift+1] > 1e-4
        # the HP->D is active either by choice or because it has to supply the shortage from the TESS.
        # thus the COP and power have to be re-calculated
        results["COPhatᴰ"][shift+1] = 7.90471 .* exp(-0.024 .* (ThpDⁱⁿ .- Tamb)) # COP nonlinear relationship
        results["Phpe"][shift+1] = copy(results["Q̇ₕₚᴰ"][shift+1] ./ results["COPhatᴰ"][shift+1]);
    end
    # electrical re-balance with the new HP power
    # re-extract the power of the sa (just in case we have hit the SoC limits)
    Pbess = haskey(results, "Pbess") ? results["Pbess"][shift+1] : 0;
...
function simTransitionFun_new_th!(results::Dict, data::Dict, s::modelSettings, typeOpt::DayAhead)
    # This function simulates the transition function Sₐ,ₜ₊₁ = Sₐ,ₜᴹ(Sₐ,ₜ , Pₐ,ₜ* , Wₐ,ₜ₊₁).
    # Sₐ,ₜ: state in timestep t
    # Pₐ,ₜ*: optimal setpoint in timestep t
    # Wₐ,ₜ₊₁: exogenous information in timestep t+1
...
function simTransitionFun!(results::Dict, data::Dict, s::modelSettings, typeOpt::CT_MPC)
    # This function simulates the transition function Sₐ,ₜ₊₁ = Sₐ,ₜᴹ(Sₐ,ₜ , Pₐ,ₜ* , Wₐ,ₜ₊₁).
    # Sₐ,ₜ: state in timestep t
    # Pₐ,ₜ*: optimal setpoint in timestep t
    # Wₐ,ₜ₊₁: exogenous information in timestep t+1
...
function simTransitionFun!(results::Dict, data::Dict, s::modelSettings, typeOpt::DayAhead)
    # This function simulates the transition function Sₐ,ₜ₊₁ = Sₐ,ₜᴹ(Sₐ,ₜ , Pₐ,ₜ* , Wₐ,ₜ₊₁).
    # Sₐ,ₜ: state in timestep t
    # Pₐ,ₜ*: optimal setpoint in timestep t
    # Wₐ,ₜ₊₁: exogenous information in timestep t+1
